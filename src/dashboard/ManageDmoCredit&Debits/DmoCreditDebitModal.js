...
import { fetchRetailerList, fetchDmoTransactionCode, insertDmoManualCreditDebit } from "../../api"
import Notify from "Components/notify"

export default function DmoCreditDebitModal (props) {
  return class DmoCreditDebitModal extends React.Component {
    constructor () {
      super()
      this.state = {
        retailers: [],
    ...
    handleRetailerChange (e) {
      ...
      const fetchDmoTransactionCodeReq = {
        "": ""
      }
      fetchDmoTransactionCode(fetchDmoTransactionCodeReq)
        .then(fetchDmoTransactionCodeRes => {
          const transactionCodes = fetchDmoTransactionCodeRes.ret_trans_code.map(item => ({
            value: item.id,
            text: item.code
          }))
    ...
    handleSubmit () {
      ...
      const insertDmoManualCreditDebitReq = {
        retailer_id: activeRetailer,
        code: activeTransactionCode,
        amount: parseFloat(amount),
        comment: comment
      }
      if (amount.length > 0) {
        insertDmoManualCreditDebit(insertDmoManualCreditDebitReq)
          .then(insertDmoManualCreditDebitRes => {
            Notify("success", insertDmoManualCreditDebitRes.Message)
    ...
    render () {
      return (
        <TitleAndSave handleSave={this.handleSubmit} title="Create new DMO Debit/Credit">
          <Select
