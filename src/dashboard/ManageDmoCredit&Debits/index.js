...
import { fetchDmoCreditDebitRetailers } from "../../api"
import DmoCreditDebitModal from "./DmoCreditDebitModal"
...
export default function ManageManualDebitCredit (props) {
  ...
  useEffect(() => {
    fetchDmoCreditDebitRetailers(fetchDmoCreditDebitRetailersReq)
      .then(fetchDmoCreditDebitRetailersRes => {
        setRetailersCount(fetchDmoCreditDebitRetailersRes.count)
        setRetailers(fetchDmoCreditDebitRetailersRes.retailer_manual_det || [])
        setLoading(true)
      })
  ...
  const handlePageChange = page => {
    ...
    props.history.push(`/admin/manage-dmo-credit-debit${getQueryUri(queryObj)}`)
  }
  ...
  const handleFilterSubmit = e => {
    ...
    setOffset(0)
    setActivePage(1)
    props.history.push(`/admin/manage-dmo-credit-debit?f=${filterBy}&q=${filterValue}&page=${1}`)
  }

  const reset = () => {
    props.history.push("/admin/manage-dmo-credit-debit")
    setFilterValue("")
    setFinalFilterValue("")
    setFilterBy(0)
    setActivePage(1)
  ...
  const fetchDmoCreditDebitRetailersReq = {
    limit: limit,
    offset: offset
  }

  if (finalFilterValue.length > 0) {
    fetchDmoCreditDebitRetailersReq.filter = {
